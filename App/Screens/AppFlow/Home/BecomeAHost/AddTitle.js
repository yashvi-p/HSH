...
import {setPlaceName} from '../../../../Redux/Actions/generalAction';
...
export default function AddTitle() {
  ...
  const validationSchema = Yup.object().shape({
    name: Yup.string()
      .trim()
      .max(50, GetLabel('should_be_less_than_50_character'))
      .required(GetLabel('name_is_required')),
  });
    ...
    return (
      <View style={styles.flatListContainerView}>
        <Text style={styles.hostTxt}>
          {GetLabel('lets_give_your_place_name')}
        </Text>
        <TextInput
          placeholder={GetLabel('enter_a_name')}
          value={values.name}
        ...
        <Text style={styles.countText}>{values.name.length}/50</Text>
      </View>
    );
  };
    ...
    <View style={styles.container}>
      <Formik
        ...
        onSubmit={(values, {resetForm}) => {
          dispatch(setPlaceName(values.name));
